...
export default class MarkSelector extends React.Component {
	...
	render() {
		...
		if(!this._firstLoad && this.props.products !== undefined && this.props.products.length > 0){
			...
			var bulmaBtnInf = ["button is-fullwidth is-info btnMargin","button is-fullwidth is-warning btnMargin","button is-fullwidth is-danger  btnMargin","button is-fullwidth is-success btnMargin","button is-fullwidth is-link btnMargin"];
			var ind = 0;
			uniqs = uniqs.map(k=> {
				if(ind > 4) ind=0;
