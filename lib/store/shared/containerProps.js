...
var setQuery = function setQuery(history, query) {
	if (history && history.location) {
		var newLocation =
			history.location.pathname + '?' + _queryString2.default.stringify(query);
...
var mapDispatchToProps = (exports.mapDispatchToProps = function mapDispatchToProps(
	...
	return {
		...
		setPartFilter: function setPartFilter(part) {
			dispatch((0, _actions.setCarPartFilterMethod)(part, ownProps.history));
		},
		setSearchedText: function setSearchedText(searchedText) {
			dispatch(
				(0, _actions.setSearchedTextMethod)(searchedText, ownProps.history)
			);
		},
		setSearch: function setSearch(search) {
			var query = _queryString2.default.parse(ownProps.history.location.search);

			var carPartFilter = dispatch((0, _actions.getPartFilter)());
			var searchedText = dispatch((0, _actions.getSearchedText)());
			var allfilter = '';
